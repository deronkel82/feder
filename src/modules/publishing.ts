...
function sections(
  ...
  if (o.titlePage)
    add(
      'title',
      'Schmutztitel',
      `<section class="front title-page">${o.halfTitleSeriesPosition === 'above' ? series : ''}${o.halfTitleTitle ? `<h1>${escape(p.title)}</h1>` : ''}${o.halfTitleAuthor && !o.anonymous ? `<p>${escape(p.author)}</p>` : ''}${p.series.enabled && o.halfTitleVolume ? `<p>Band ${escape(p.series.volume)}</p>` : ''}${o.halfTitleSeriesPosition === 'below' ? series : ''}</section>`,
    );
