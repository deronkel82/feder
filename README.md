...
Unter dem Schreibfeld erscheinen Wort- und Zeichenzahl für alle Projektarten, einschließlich Sonstiges und Fokusmodus. Bei mehreren Textabschnitten werden zusätzlich die Werte der aktuellen Szene beziehungsweise des aktuellen Kapitels angezeigt. Gezählt wird ausschließlich der Manuskripttext, einschließlich Leerzeichen und normalisierter Absatzumbrüche; Titel, Zusammenfassungen und Notizen zählen nicht mit. Zeichen werden als Unicode-Codepunkte gezählt. Zwischen nichtleeren Abschnitten zählen zwei Absatzumbrüche.

Kurzgeschichten bleiben standardmäßig ein zusammenhängender Text. Unter **Projektverwaltung → Projektart / Textstruktur → Szenen verwenden (ohne Kapitel)** oder beim Anlegen kann die Szenenmethodik eingeschaltet werden. Der vorhandene Text bleibt dabei als erste Szene erhalten. Eine Kurzgeschichte hat eine flache Szenenliste, keine Kapitel, Akte oder Reihenverwaltung. Szenen können über „+“ angelegt, am Schreibfeld umbenannt, über „…“ gelöscht und in der Werkstatt oder Szenenwand umgeordnet werden. Ideen können als neue geplante Szene übernommen werden.

Beim Abschalten werden die Szenen in ihrer Reihenfolge verbunden; der ursprüngliche Stand wird als Version gesichert. Erneutes Einschalten stellt frühere Szenengrenzen nicht automatisch wieder her. Markdown, HTML/Druck/PDF und EPUB enthalten alle Szenen. Vor Austausch oder Google-Synchronisierung einer Kurzgeschichte mit mehreren Szenen alle Geräte auf mindestens 0.15 aktualisieren; ältere Versionen unterstützen diese Struktur nicht.
